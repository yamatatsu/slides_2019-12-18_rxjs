...
> 私は最近アーキテクチャ設計の失敗をしました。失敗辛い。マイクロサービス(オライリー)曰く、イベント駆動的でコレオグラフィ的であるほど変更に強くなると。そしてまたマイクロサービス(オライリー)曰く、始めっからマイクロサービスにすべきでなく最初はモノリスにしてDDD(エリック・エヴァンス)でいうモジュールで分割しておくべきだと。じゃあそのモジュール同士がrequest/responseで繋がっている場合、切り出す際の傷が多いはずだ。マクロがイベント駆動を望むならミクロもまたイベント駆動を愛すはずだ。フラクタル最高。
